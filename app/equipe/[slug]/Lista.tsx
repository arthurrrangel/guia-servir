...
/* A dica que separa duas pessoas de mesmo nome, em no máximo duas funções.
   Ela é a REDE, não o cinto: assim que o nome inteiro chega — por qualquer um
   dos dois caminhos — ele resolve sozinho e a dica sai de cena. Dois
   diferenciadores na mesma linha seriam ruído permanente.
   Mora dentro do componente porque precisa enxergar o mapa `nomes`. */
...
export default function Lista({ nomes }: { nomes: Record<string, string> }) {
  const { slug } = useParams<{ slug: string }>();

  /* o nome inteiro: da própria linha quando a migração 42 já rodou, do mapa do
     servidor enquanto não rodou, e o primeiro nome como último degrau */
  const nomeNaLista = (p: Pessoa) =>
    emNome(p.nome_completo || nomes[p.voluntario_id] || p.primeiro_nome);
  const dicaDe = (p: Pessoa) => {
    if (p.nome_completo || nomes[p.voluntario_id]) return null;
