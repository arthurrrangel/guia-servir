...
export default function Movimento({ semRevelar = false }: { semRevelar?: boolean }) {
  useEffect(() => {
    ...
    /* 16/09/2026: SAÍRAM DAQUI o anel do cursor, o foco de luz que seguia o
       ponteiro nas faixas escuras e o ímã do botão principal. Eram os três
       efeitos que todo site de produto de 2024 tem, e no celular nenhum dos
       três existe: só custavam três listeners de mousemove. A assinatura de
       movimento do site é o que sobrou: a revelação por seção, a profundidade
       da foto ao rolar e o herói que recua com o scroll. */

    /* o que nasce depois (as faixas das páginas que buscam dados no cliente)
       entra na lista de fotos acima, senão foto nova não anda */
