...
export default function Movimento({ semRevelar = false }: { semRevelar?: boolean }) {
  useEffect(() => {
    ...
    /* 17/09/2026 · EDITORIAL CLARO, MOVIMENTO DISCRETO (escolha do Arthur).
       Saíram daqui o anel do cursor (1c), o foco de luz nas faixas escuras
       (2) e o ímã do botão (3): nada mais segue o ponteiro. As faixas escuras
       viraram claras (editorial.css), então o foco não teria onde acender.
       Ficam a profundidade na rolagem (1b) e o mapa que não carrega. */

    /* o que nasce depois (as faixas das páginas que buscam dados no cliente)
       entra na lista de fotos, senão foto nova não anda */
