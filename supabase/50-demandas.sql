/* ESTE ARQUIVO E PASSADO. A TRANCA ESTA AQUI PORQUE ELE PODE DESFAZER.
   ...
   O que este arquivo consegue reverter, se rodar fora de hora:
     dem_abrir (a 52 refez), dem_lista (a 56 refez), dem_mover (a 52 refez)

   Por isso ele se recusa a rodar num banco que ja passou da 51. Aplicado na
...
/* =============================================================================
   01 · DEMANDAS — a estrutura inteira, numa migração só
   18/09/2026.
   ...
   As tabelas ficam no esquema `demandas`, que NÃO é exposto na API. A chave
   pública do app não alcança nenhuma delas. A única porta são as funções
   `public.dem_*` abaixo, todas `security definer`, e cada uma confere quem
   está chamando antes de fazer qualquer coisa. É mais fechado que RLS: não
   existe consulta livre para alguém descobrir como driblar.

   Mesmo projeto Supabase do GUIA Servir, de propósito:
     · o plano grátis PAUSA projeto parado 7 dias, e um sistema de demandas
       usado de vez em quando pausaria sozinho. Aqui ele pega carona na
       atividade diária do outro;
     · quem já entra no GUIA Servir com e-mail e senha entra aqui com a MESMA
       senha, porque o Auth é do projeto. Zero cadastro novo para a liderança;
     · um banco só para fazer backup.

   Nada aqui ESCREVE em `public`. A ligação com `public.pessoas` é só de
   leitura, por `membros.pessoa_id`, e sem chave estrangeira — assim esta
   migração roda também num projeto vazio.

   ---------------------------------------------------------------------------
   AS 11 REGRAS DO DOCUMENTO VIRAM CHECK E GATILHO, NÃO CÓDIGO DE TELA

   Regra escrita em JavaScript é regra que a próxima tela esquece. Cada uma
   das regras da seção 8 do PDF está abaixo como restrição do banco:

     "toda demanda precisa de setor solicitante"      → not null
     "toda demanda precisa de setor responsável"      → not null
     "prazo desejado OU justificativa"                → check ck_prazo
     "urgente precisa informar o impacto"             → check ck_urgente
     "evento precisa estar vinculado a uma data"      → check ck_evento
     "a conclusão deve informar o que foi realizado"  → check ck_conclusao
     "toda alteração relevante fica no histórico"     → gatilho fn_historico
     "sem movimentação vira pendente de atenção"      → coluna mexida_em
     "concluídas podem ser reabertas"                 → ação reabrir + contador
     "compras devem conter orçamento quando possível" → campo, sem check
                                                        ("quando possível" não
                                                         é restrição)
     "o solicitante recebe notificação"               → fora do banco: a tela
                                                        monta o link do
                                                        WhatsApp. Ver README.
   ============================================================================= */
...
-- =============================================================== SETORES ===
/* Setor é a unidade do documento: quem pede e quem atende. `atende` separa os
   seis setores que RECEBEM demanda dos ministérios, que só pedem. Todo mundo
   pode pedir. */
...
-- =============================================================== MEMBROS ===
/* Uma linha por pessoa que usa o sistema. Duas portas de entrada, as mesmas
   do GUIA Servir:
     · `auth_email` → entra com e-mail e senha (Supabase Auth);
     · `token`      → link pessoal, para quem não tem login.
   `pessoa_id` liga em `public.pessoas` quando a pessoa já existe lá, para não
   virar um segundo cadastro da mesma gente. É opcional: funcionário da igreja
   que nunca serviu em ministério entra aqui e pronto. */
...
/* AQUI MORA A TRIAGEM.
   ...
   A etapa 2 do documento ("a liderança analisa para verificar qual setor deve
   atender") é a etapa que mata sistema de chamado em organização pequena:
   ninguém tem "triador" como função. Então a categoria já carrega o setor que
   atende, se exige aprovação e se pede orçamento. A demanda nasce roteada, e
   o gestor redireciona quando o automático errar.

   `prazo_padrao_dias` sugere a data no formulário; ninguém é obrigado a
   aceitar. */
...
create table if not exists demandas.demandas (
  ...
  /* identificação e detalhamento (seções 2 do PDF) */
...
/* --- as regras da seção 8, como restrição --------------------------------- */
do $$ begin
...
exception when duplicate_object then null; end $$;
do $$ begin
  /* "toda demanda precisa ter um prazo desejado OU uma justificativa" */
...
exception when duplicate_object then null; end $$;
do $$ begin
  /* "demandas urgentes precisam informar o impacto da urgência" */
...
exception when duplicate_object then null; end $$;
do $$ begin
  /* "demandas relacionadas a eventos devem estar vinculadas a uma data" */
...
exception when duplicate_object then null; end $$;
do $$ begin
  /* "a conclusão deve informar o que foi realizado" */
...
exception when duplicate_object then null; end $$;
do $$ begin
  /* travada sem motivo é só uma etiqueta. O documento é explícito: "os status
     devem refletir o que está acontecendo". */
...
-- ================================================================ EVENTOS ===
/* O histórico. `interno` marca comentário que o solicitante não vê. */
...
/* =============================================================================
   O GATILHO DO HISTÓRICO

   "Toda alteração relevante precisa ficar registrada no histórico" é a regra
   mais fácil de quebrar sem perceber: basta alguém escrever um UPDATE novo em
   algum lugar e esquecer de gravar o evento. Aqui o banco grava sozinho.

   Quem está agindo chega por `set_config('demandas.membro', …, true)`, que as
   funções abaixo definem no começo. É local à transação.
   ============================================================================= */
...
/* =============================================================================
   ...
   Uma pergunta, uma resposta, para as duas portas — token pessoal ou e-mail
   do login. É a mesma ideia do `quem_sou` do GUIA Servir.
   ============================================================================= */
...
/* Pode ver esta demanda? Gestor e admin veem tudo; os demais veem o que
   abriram, o que o setor deles pediu e o que o setor deles atende. */
...
/* =============================================================================
   AS FUNÇÕES QUE O APP CHAMA. Tudo passa por aqui.
   ============================================================================= */
...
-- 6 ------------------------------------------- o funil único das ações ---
/* Todas as transições passam por aqui. Um lugar só para conferir permissão é
   a mesma ideia do `candidatos()` do GUIA Servir: se a checagem estivesse
   espalhada, a próxima tela esqueceria uma. */
...
-- 7 ------------------------------------------------------------- números ---
/* Os 14 indicadores da seção 9, numa chamada só. */
...
-- 9 --------------------------------- a lista de gente, para o admin ver ---
...
/* -- um acento-fora simples, porque `unaccent` é extensão e pode não estar ---- */
...
/* ============================================================== PERMISSÕES ==
   Nenhuma tabela é alcançável pela chave pública. Só estas funções. */
...
/* =============================================================================
   SEMENTE: os setores e as 44 categorias da seção 3 do PDF, palavra por
   palavra, cada uma já apontando para o setor que atende.

   `on conflict do nothing`: rodar de novo não duplica e não desfaz ajuste que
   a liderança tenha feito na tela.
   ============================================================================= */
...
/* =============================================================================
   DEPOIS DE RODAR: o primeiro admin

   Sem isto ninguém entra. Troque o nome, o e-mail do login e o telefone:

     insert into demandas.membros (nome, auth_email, telefone, papel, setor_id, token)
     values ('Arthur Rangel', 'arthurrangel427@gmail.com', '5500000000000', 'admin',
             (select id from demandas.setores where slug = 'secretaria'),
             encode(gen_random_bytes(12), 'hex'));

   `auth_email` tem que ser o MESMO e-mail com que a pessoa entra no Supabase
   Auth deste projeto. Quem já entra no GUIA Servir usa a mesma senha.

   ROLLBACK
     drop schema demandas cascade;
     drop function if exists public.dem_quem_sou(text), public.dem_bases(text),
       public.dem_abrir(text,jsonb), public.dem_lista(text,jsonb),
       public.dem_ver(text,int), public.dem_mover(text,int,text,jsonb),
