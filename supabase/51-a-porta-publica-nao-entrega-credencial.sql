/* ESTE ARQUIVO E PASSADO. A TRANCA ESTA AQUI PORQUE ELE PODE DESFAZER.
   ...
   O que este arquivo consegue reverter, se rodar fora de hora:
     candidatar (a 63 e a 64 refizeram: reaplicar aqui devolve o token no corpo da resposta publica)
...
declare
  ...
  v_fn uuid;
  /* 19/09/2026: esta conferência passava '{}' como respostas e morria com
     PERGUNTA_OBRIGATORIA assim que o ministério tivesse uma pergunta marcada
     como obrigatória — o que é uma configuração normal, não um defeito. O
     teste passou a montar as respostas a partir das perguntas que existem,
     em vez de supor que não existem. */
  v_resp jsonb;
begin
  ...
  select coalesce(jsonb_object_agg(q.id::text, 'resposta de teste'), '{}'::jsonb)
    into v_resp
    from perguntas q
  ...
  r := candidatar(v_slug, 'Teste Um Cinquentaeum', v_tel_novo, null,
                  array[(select nome from funcoes where id = v_fn)], v_resp);
